...
( function( $ ) {
	$.fn.paginate = function( options ) {
		...
		obj.append(
			$( '<div/>' ).addClass( options.pagination_class + "__controls mx-5" ).append(
		...
		function createPaginationControls( pages ) {
			...
			if ( options.prev_next == true ) {
				obj.find( "." + options.pagination_class ).append( '<li class="prev btn btn-dark btn-lg"><a>' + options.prev_text + '</a></li>' );
			}
			// For each div (slide) add a link in span for controls
			for ( var i = 0; i < pages; i++ ) {
				obj.find( "." + options.pagination_class ).append( '<li class="btn btn-dark btn-lg"><a href="#' + ( i + 1 ) + '">' + ( i + 1 ) + '</a></li>' );
			};

			// Add Next Button
			if ( options.prev_next == true ) {
				obj.find( "." + options.pagination_class ).append( '<li class="next btn btn-dark btn-lg"><a>' + options.next_text + '</a></li>' );
			}
		}
